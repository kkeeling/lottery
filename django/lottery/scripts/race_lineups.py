...
def run():
    ...
    start = time.time()
    p1 = df_slate_lineups.merge(player_outcomes, how='left', left_on=0, right_on='slate_player_id')#.merge(player_outcomes, how='inner', left_on=1, right_on='slate_player_id').merge(player_outcomes, how='inner', left_on=2, right_on='slate_player_id').merge(player_outcomes, how='inner', left_on=3, right_on='slate_player_id').merge(player_outcomes, how='inner', left_on=4, right_on='slate_player_id').merge(player_outcomes, how='inner', left_on=5, right_on='slate_player_id')
    print(f'  Merge took {time.time() - start}s')
    start = time.time()
    p2 = df_slate_lineups.merge(player_outcomes, how='left', left_on=1, right_on='slate_player_id')
    print(f'  Merge took {time.time() - start}s')
    start = time.time()
    p3 = df_slate_lineups.merge(player_outcomes, how='left', left_on=2, right_on='slate_player_id')
    print(f'  Merge took {time.time() - start}s')
    start = time.time()
    p4 = df_slate_lineups.merge(player_outcomes, how='left', left_on=3, right_on='slate_player_id')
    print(f'  Merge took {time.time() - start}s')
    start = time.time()
    p5 = df_slate_lineups.merge(player_outcomes, how='left', left_on=4, right_on='slate_player_id')
    print(f'  Merge took {time.time() - start}s')
    start = time.time()
    p6 = df_slate_lineups.merge(player_outcomes, how='left', left_on=5, right_on='slate_player_id')
    print(f'  Merge took {time.time() - start}s')
    start = time.time()
    print(p1)
    print(p2)
