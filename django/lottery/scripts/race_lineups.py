...
def run():
    ...
    slate_lineup_scores = df_matchups['slate_lineup_id'].map(lambda x: numpy.array(df_slate_lineups.loc[x]['scores'])).to_numpy()
    field_scores = df_matchups['field_lineup_id'].map(lambda x: numpy.array(df_field_lineups.loc[x]['scores'])).to_numpy()
    diffs = slate_lineup_scores - field_scores
    del slate_lineup_scores
    del field_lineups
    gc.collect()
    
    df_matchups['win_rate'] = [numpy.count_nonzero(x > 0.0) / build.sim.iterations for x in diffs]
    del diffs
    gc.collect()
