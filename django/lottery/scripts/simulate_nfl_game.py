    ...
    for game in [slate.games.all()[4]]:
        try:
            ...
            for _ in range(0, 1):
                ...
                s = [
                    float(home_qb.stdev),
                    float(home_rb1.stdev),
                    float(home_rb2.stdev),
                    float(home_wr1.stdev),
                    float(home_wr2.stdev),
                    float(home_wr3.stdev),
                    float(home_te.stdev),
                    float(home_dst.stdev),
                    float(away_qb.stdev),
                    float(away_rb1.stdev),
                    float(away_rb2.stdev),
                    float(away_wr1.stdev),
                    float(away_wr2.stdev),
                    float(away_wr3.stdev),
                    float(away_te.stdev),
                    float(away_dst.stdev),
                ]

                # b = np.linalg.multi_dot([s, r_df, s])
                # c = scipy.linalg.cholesky(b, lower=True)
                ...
                #     scipy.stats.skewnorm.rvs(96.20556978252375, loc=float(home_rb1.projection), scale=float(home_rb1.stdev)),
                #     scipy.stats.skewnorm.rvs(96.20556978252375, loc=float(home_rb2.projection), scale=float(home_rb2.stdev)),
                #     scipy.stats.skewnorm.rvs(100226585.84323588, loc=float(home_wr1.projection), scale=float(home_wr1.stdev)),
                #     scipy.stats.skewnorm.rvs(100226585.84323588, loc=float(home_wr2.projection), scale=float(home_wr2.stdev)),
                #     scipy.stats.skewnorm.rvs(100226585.84323588, loc=float(home_wr3.projection), scale=float(home_wr3.stdev)),
                #     scipy.stats.skewnorm.rvs(45.31937198302796, loc=float(home_te.projection), scale=float(home_te.stdev)),
                #     scipy.stats.skewnorm.rvs(4.085406579290768, loc=float(home_dst.projection), scale=float(home_dst.stdev)),
                #     scipy.stats.skewnorm.rvs(1.4132959978590718, loc=float(away_qb.projection), scale=float(away_qb.stdev)),
                #     scipy.stats.skewnorm.rvs(96.20556978252375, loc=float(away_rb1.projection), scale=float(away_rb1.stdev)),
                #     scipy.stats.skewnorm.rvs(96.20556978252375, loc=float(away_rb2.projection), scale=float(away_rb2.stdev)),
                #     scipy.stats.skewnorm.rvs(100226585.84323588, loc=float(away_wr1.projection), scale=float(away_wr1.stdev)),
                #     scipy.stats.skewnorm.rvs(100226585.84323588, loc=float(away_wr2.projection), scale=float(away_wr2.stdev)),
                #     scipy.stats.skewnorm.rvs(100226585.84323588, loc=float(away_wr3.projection), scale=float(away_wr3.stdev)),
                #     scipy.stats.skewnorm.rvs(45.31937198302796, loc=float(away_te.projection), scale=float(away_te.stdev)),
                #     scipy.stats.skewnorm.rvs(4.085406579290768, loc=float(away_dst.projection), scale=float(away_dst.stdev)),
                # ]
                X = scipy.stats.yeojohnson(x)

                D = np.diag(s)  # diag(stdev)
                print(pd.DataFrame(D))
                S = np.linalg.multi_dot([D, r_df.to_numpy(), D])  # covariance matrix
                print(pd.DataFrame(S))
                C = np.linalg.cholesky(S)  # decomposed covariance matrix
