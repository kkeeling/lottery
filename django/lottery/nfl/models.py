...
class SlateBuild(models.Model):
    ...
    backtest = models.ForeignKey('BacktestSlate', related_name='builds', on_delete=models.SET_NULL, null=True, blank=True)
    ...
    def update_projections(self, replace=True):
        ...
        for player in self.slate.players.all():
            ...
            if hasattr(player, 'projection'):
                ...
                projection.team_total = player.projection.team_total
                projection.game_total = player.projection.game_total
                projection.spread = player.projection.spread
                projection.save()
            else:
        ...
        for stack in stacks:
            ...
            for (index, lineup) in enumerate(lineups):
                SlateBuildLineup.objects.create(
                    ...
                    qb=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[0].id),
                    rb1=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[1].id),
                    rb2=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[2].id),
                    wr1=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[3].id),
                    wr2=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[4].id),
                    wr3=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[5].id),
                    te=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[6].id),
                    flex=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[7].id),
                    dst=BuildPlayerProjection.objects.get(slate_player__player_id=lineup.players[8].id),
...
class Backtest(models.Model):
    ...
    def execute(self):
        ...
        For each slate in backtest
            1. Create a build, if necessary
            2. Copy default projections
            3. Find in play
            4. Create Groups
            5. Create Stacks
            6. Clean Stacks
