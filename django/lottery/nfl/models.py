...
PROJECTION_SITES = (
    ...
    ('4for4_sat', '4for4 Saturday'),
    ('awesemo_own_sat', 'Awesemo Saturday Ownership'),
    ('etr_sat', 'Establish The Run Saturday'),
    ('linestar_sat', 'Linestar Saturday'),
    ('rg_sat', 'Rotogrinders Saturday'),
    ('4for4_sat_mon', '4for4 Sat-Mon'),
    ('awesemo_own_sat_mon', 'Awesemo Sat-Mon Ownership'),
    ('linestar_sat_mon', 'Linestar Sat-Mon'),
    ('rg_sat_mon', 'Rotogrinders Sat-Mon'),
    ('4for4_sat_sun', '4for4 Sat-Sun'),
    ('awesemo_own_sat_sun', 'Awesemo Sat-Sun Ownership'),
    ('linestar_sat_sun', 'Linestar Sat-Sun'),
    ('rg_sat_sun', 'Rotogrinders Sat-Sun'),
...
class SlatePlayer(models.Model):
    ...
    def __str__(self):
        if self.fantasy_points is None:
            return '{} {} ${} (vs. {}) -- {}%'.format(self.team, self.name, self.salary, self.get_opponent(), round(self.get_ownership_projection()*100, 2))
        return '{} {} ${} (vs. {}) -- {}% -- {}'.format(self.team, self.name, self.salary, self.get_opponent(), round(self.get_ownership_projection()*100, 2), self.fantasy_points)
...
class BuildPlayerProjection(models.Model):
    ...
    qb_stack_only = models.BooleanField(default=False, verbose_name='SwQB', help_text='Generate QB stacks with this player')
    opp_qb_stack_only = models.BooleanField(default=False, verbose_name='SwOQB', help_text='Generate Opp QB stacks with this player')
    disallow_ministack = models.BooleanField(default=False, verbose_name='No Mini')
    use_as_antileverage = models.BooleanField(default=False, verbose_name='Anti Lev')
